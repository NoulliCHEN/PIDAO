...
def set_para(lr):
    ...
    kp = 1.2 * lr * (1 + momentum * lr) / lr ** 2
    ki = 0.2
    kd = 2.5
    kp_a = 8*(1 + momentum * lr) / lr
    kd_a = 1
    ki_a = 0.001
...
def Optimizers(x, opt_name, s):
    ...
    Optimizer_set = {
        ...
        'PIDAO_SI_AAdRMS': PIDAccOptimizer_SI_AAdRMS([x], lr=s, kp=kp_a, ki=ki_a, kd=kd_a, momentum=momentum,
                                                     beta1=0.9987),
        'Adam': optim.Adam([x], lr=8*s),
        # 'Nesterov': optim.SGD([x], lr=1.2 * s, momentum=equivalent_momentum, nesterov=bool(s)),
        'GDM': optim.SGD([x], lr=1.2 * s, momentum=equivalent_momentum),
    }
...
if __name__ == '__main__':
    ...
    xx = torch.linspace(-abs(x1_0) - .2, 1.3, 250)
    yy = torch.linspace(-.66, 2.6, 250)
    ...
    left, bottom, width, height = 0.7, 0.3, 0.2, 0.3
    axes1_1 = fig.add_axes([left, bottom, width, height])
    ...
    axes1_1.tick_params(labelsize=fontsize-10)
    labels = axes1_1.get_xticklabels() + axes1_1.get_yticklabels()
    [label.set_fontname('Times New Roman') for label in labels]
    axes1_1.grid(color='grey', linestyle='-.', linewidth=2, alpha=0.6)
    axes1_1.set_xlim([1700, 4000])
    axes1_1.set_ylim([3.5 * 10 ** -12, 1 * 10 ** -10])
    fig.subplots_adjust(left=0.05,
                        ...
                        hspace=0.35)
    plt.show()
    fig.savefig('figure/discrete_comparison_rosenbrock.svg', bbox_inches='tight', dpi=600)
